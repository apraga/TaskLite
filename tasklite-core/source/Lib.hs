  ...
  let isTag = ("+" `T.isPrefixOf`)
      ...
      createdUtcMb =
        metadata
          ...
          & P.lastMay
            >>= parseUtc
   in (body, tags, dueUtcMb, createdUtcMb)
...
addTask conf connection bodyWords = do
  ...
  let (body, tags, dueUtcMb, createdUtcMb) = parseTaskBody bodyWords
      importTaskDraft =
        ImportTask
          { task =
              emptyTask
  ...
  case insertedTasks of
    [insertedTask] -> do
      ...
      let postAddHookMsg :: Doc AnsiStyle
          postAddHookMsg =
            ( postAddResults
                <&> \case
...
logTask conf connection bodyWords = do
  (ulid, modified_utc, effectiveUserName) <- getTriple conf
  let (body, extractedTags, dueUtcMb, createdUtcMb) = parseTaskBody bodyWords
...
waitFor conf connection duration ids = do
  docs <- forM ids $ \idSubstr ->
    execWithTask conf connection idSubstr $ \task -> do
      now <- timeCurrentP
      let nowAsText = (T.pack . timePrint conf.utcFormat) now
          threeDays =
...
formatTaskForInfo conf now (taskV, tags, notes) =
  let mkGreen = annotate (color Green)
        ...
        let scoreParts =
            ...
            combinedText =
              scoreParts
                ...
                & vcat
         in (highestScore, ulid, combinedText)
...
showAtPrecision numOfDigits number =
  let tuple = T.breakOn "." (show number)
      clipDecimalPart =
        if snd tuple == ".0"
...
getIdLength numOfItems =
  ...
  let targetCollisionChance = 0.01 -- Targeted likelihood of id collisions
      sizeOfAlphabet = 32 -- Crockford's base 32 alphabet
   in ceiling (logBase sizeOfAlphabet (numOfItems / targetCollisionChance)) + 1
    ...
    Just (filterExps, _) -> do
      ...
      if P.length errors > 0
        then
          ...
          let isStateExp = \case (HasStatus _) -> True; _ -> False
              filterExpWithOpen =
...
runFilter conf now connection exps availableLinesMb = do
  let parserResults = readP_to_S filterExpsParser $ T.unpack $ unwords exps
      filterHelp =
...
getProgressBar maxWidthInChars progress =
  let barWidth = floor (progress * fromInteger maxWidthInChars)
...
formatTagLine conf maxTagLength (tag, open_count, closed_count, progress) =
  let barWidth = toInteger $ progressBarWidth conf
      progressPercentage =
        if progress == 0
          then "     "
          else
            pretty
              ...
              <+> "%"
   in fill maxTagLength (pretty tag)
...
formatTags conf tagTuples = do
  let percWidth = 6 -- Width of e.g. 100 %
...
listNotes conf connection = do
  ...
  let taskIdWidth = 7 -- TODO: Use dynamic width
